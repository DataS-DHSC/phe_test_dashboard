...
make_hist <- function(df){
  df %>%
  ggplot(aes(date, daily_cases, fill = colours))+
...
make_hist_all <- function(df){
  df %>%
    ggplot(aes(date, daily_cases, fill = area, shape = area))+
  ...
  lapply(
    function(x) {
      colours <- x %>%
        ...
        arrange(area) %>%
        distinct %>%
        mutate(colours = factor(swap_facet_colours(area))) %>%
        select(-area) %>%
        unlist
      
      x %>%
      mutate(colours = factor(area, labels = colours)) %>%
      make_hist
    }
  )
...
```{r}
ggplotly(histograms[[1]], height = 600, width = 800)
```

### 31-60

```{r}
ggplotly(histograms[[2]], height = 600, width = 800)
```

### 61-90

```{r}
ggplotly(histograms[[3]], height = 600, width = 800)
```

### 91-120

```{r}
ggplotly(histograms[[4]], height = 600, width = 800)
```

### 121-150

```{r}
ggplotly(histograms[[5]], height = 600, width = 800)
```
