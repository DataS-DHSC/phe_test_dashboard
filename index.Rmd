...
df <- df %>%
  ...
  bind_rows(
    df %>%
      group_by(date) %>%
      mutate_at(vars(contains('cases')), ~ round(sum(., na.rm = TRUE))) %>%
      mutate(
        area = 'All local authorities',
        code = NA
      ) %>%
      ungroup %>%
      distinct
...
pos_tests_last_week <- df %>%
  filter(area != 'All local authorities') %>%
  data_for_barplots(
...
Scatter plots display, for each Local Authority (LA), change in positive tests, in two ways: (i) change in the _total_ number of positive tests since the start of the pandemic between last week and the week before; and (ii) change in the number of _weekly_ positive tests between last week and the week before.
...
- Histogram of daily positive Covid-19 tests by Local Authority (LA)
  - At first view, the plot displays the histograms from All local authorities on top of each other
